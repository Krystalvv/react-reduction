...
const SalesTotal = () => {
  ...
  const [graph, setGraph] = useState(JSON.parse(localStorage.getItem('graphIndex')));
  const data = graph === 0 ? DayData() : graph === 1 ? WeekData() : graph === 2 ? MonthData() : DayData()
  ...
  const getData = () => {
    ...
    switch (graph) {
      case 0: data = DayData()
        break;
      case 1: data = WeekData()
        break;
      case 2: data = MonthData()
        break;
      default: data = DayData()
        break;
    }
    return data;
  }
    ...
    <Page category_buttons={sales_url}>
      ...
      <div style={{ display: graph !== 3 ? "block" : "none", padding: "5px 5px 0 5px", backgroundColor: "white" }}>
        ...
        <div style={{ padding: "20px", margin: "10px", marginBottom: "0", border: "1px solid #d9d9d9" }}>
          ...
          <Bar height="100%" data={getData()} options={barChartOptions} />
        </div>
      </div>
